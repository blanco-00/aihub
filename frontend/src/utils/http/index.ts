    ...
    instance.interceptors.response.use(
      (response: PureHttpResponse) => {
        ...
        if (requestStartTime) {
          ...
          const performanceEntries = performance.getEntriesByName($config.url || '', 'resource') as PerformanceResourceTiming[];
          const resourceTiming = performanceEntries[0];
          
          if (resourceTiming) {
            // 如果排队时间很长，特别警告
            const queuingTime = resourceTiming.startTime - resourceTiming.fetchStart;
            if (queuingTime > 1000) {
              console.error(`[HTTP严重警告] ${requestId} 请求排队时间过长: ${queuingTime.toFixed(2)}ms, URL: ${$config.url}`);
            }
            
            // 如果连接建立时间很长，特别警告
            const connectTime = resourceTiming.connectEnd - resourceTiming.connectStart;
            if (connectTime > 1000) {
              console.error(`[HTTP严重警告] ${requestId} 连接建立时间过长: ${connectTime.toFixed(2)}ms, URL: ${$config.url}`);
            }
          }
          
          // 只记录超过1秒的请求
          if (totalTime > 1000) {
            console.warn(`[HTTP性能警告] ${requestId} 请求总耗时过长: ${totalTime.toFixed(2)}ms, URL: ${$config.url}`);
          }
        }
          ...
          if (isAuthError) {
            ...
            if (tokenData?.refreshToken && !PureHttp.isRefreshing) {
              try {
                ...
                if (originalRequest) {
                  originalRequest.headers["Authorization"] = formatToken(res.data.accessToken);
              ...
              useUserStoreHook().logOut();
              message(errorMessage || transformI18n($t("login.pureLoginExpired")), {
                type: "warning"
              });
            }
          } else {
