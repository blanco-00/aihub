...
export const getUserList = (params?: UserListParams) => {
  return http.request<Result<PageResult<UserInfo>>>("get", "/api/users", {
    params,
  });
};
...
export const toggleUserStatus = (id: number, status: number) => {
  return http.request<Result<void>>("put", `/api/users/${id}/status`, {
    params: { status },
  });
};
...
export const getMine = (data?: object) => {
  return http
    .request<Result<UserInfo>>("get", "/api/auth/me", { data })
    .then((response: any) => {
      // 转换后端数据格式为前端需要的格式
...
export const getMineLogs = (params?: { current?: number; size?: number }) => {
  return http.request<Result<PageResult<SecurityLogInfo>>>(
    "get",
