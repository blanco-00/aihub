...
- [快速开始指南](./quick-start.md) - 快速初始化项目、启动应用
- [模块架构规划](./module-architecture.md) - 后端模块划分方案和迁移计划
- [数据库配置说明](./config.md) - 数据库连接配置和安全指南
- [数据库设计文档](./database.md) - 数据库选型、设计规范、核心业务表设计
- [系统初始化文档](./initialization.md) - 数据库初始化、超级管理员创建
- [日志配置文档](./logging.md) - 日志配置和日志等级使用

### 功能模块设计文档

> **说明**：功能模块设计文档包含该模块的数据库表设计和功能设计，以 [功能清单](../features.md) 为起点。

- [用户与权限模块设计](./user-permission.md) - 用户、角色、部门的数据库设计和功能设计
- [菜单管理模块设计](./menu.md) - 菜单管理的数据库设计和功能设计
...
1. **环境准备**: JDK 17+、Maven 3.6+、Docker Desktop、Node.js 20.19.0+（或 22.13.0+）、pnpm >= 9
2. **启动 Docker 开发环境**: 启动 MySQL 和 Redis 容器
3. **启动服务**: 启动后端和前端服务（后端自动连接数据库）
4. **系统初始化**: 通过页面初始化数据库表结构和创建超级管理员

### 相关文档

- **[快速开始指南](./quick-start.md)** - 完整的初始化流程和详细说明
- **[数据库配置说明](./config.md)** - 数据库连接配置和安全指南
- **[系统初始化文档](./initialization.md)** - 系统初始化的详细流程和技术实现
